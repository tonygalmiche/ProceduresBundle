...
namespace OVE\ProceduresBundle\Command;
...
class ManuelCommand extends ContainerAwareCommand

{
    protected function configure()
    {
        $this
            ->setName('procedures:manuel')
            ->setDescription('Générer le manuel de procédures')
    ...
    {
        $output->writeln("Manuel non généré => Non implémenté");
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach($entities as $v) {
          ...
          //$form = $entity->createForm(new proceduresType(), $entity);
          ...
          $url="https://procedures-demo.fondation-ove.fr";

          //** Diagramme ********************************************************
          $editId = $id."-diagramme";
          //$existingFiles = $this->get('punk_ave.file_uploader')->getFiles(array('folder' => 'attachments/' . $editId));
          $existingFiles = $this->getContainer()->get('punk_ave.file_uploader')->getFiles(array('folder' => 'attachments/' . $editId));
        ...
        $output->writeln("Manuel généré avec succès");
