function PunkAveFileUploader(options)
{
  var self = this,
    ...
    $el = $(options.el),
    uploaderTemplate = _.template($.trim($(options.uploaderTemplate).html()));
  ...
  var fileTemplate = _.template($.trim($(options.fileTemplate).html())),
  ...
  self.errorCallback = 'errorCallback' in options ? options.errorCallback : function( info ) { 
    if (window.console && console.log) { 
      console.log(info) 
      if(info.error=="acceptFileTypes")  alert("Format de fichier non autorisé");
      if(info.error=="maxNumberOfFiles") alert("Nombre maximum de fichiers dépassé");
    } 
  },
  ...
  editor.fileupload({
    ...
    /*uploadTemplateId: 'toto',*/



    progressall: function (e, data) {
        var progress = parseInt(data.loaded / data.total * 100, 10);
        //console.log(options.el+" : "+progress);

        $(options.el).find('.progress-bar').css(
            'width',
            progress + '%'
        );

        /*
        $(options.el+' progress-bar').css(
            'width',
            progress + '%'
        );
        */
  ...
  function appendEditableImage(info)
  {
    ...
    if ('undefined' == typeof info.size) info.size=0;
    if ('undefined' == typeof info.type) info.type="";
